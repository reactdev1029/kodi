...
let fittingsList = ['Bucket', 'Hose', 'Drill', 'Compactor'];

class RentSearchResults extends React.Component {
  constructor() {
    super();
    this.state = {};
  }
  ...
  setInitialState(mans_models) {
    ...
    let fittings = '';
    fittingsList.map(fitting => {
      if (this.state[fitting] === undefined) {
        fittings += `${fitting.toLowerCase()},`;
        this.setState({ [fitting]: true, fittings });
      }
    });
  }
  ...
  handleCheckChange = name => (event, checked) => {
    ...
    if (checked) {
      this.setState({
        fittings: `${this.state.fittings}${name.toLowerCase()},`,
      });
    } else {
      this.setState({
        fittings: this.state.fittings.replace(`${name.toLowerCase()},`, ''),
      });
    }
  };
  ...
  render() {
    const { o_countries, fittings } = this.state;
    ...
    let fitting_state = {};
    fittingsList.map(fitting => {
      fitting_state[fitting] = this.state[fitting];
    });
    ...
    let fittings_value = '';
    if (fittings !== undefined && fittings !== '') {
      fittings_value = fittings.slice(0, -1);
    }
...
export default connect(mapStateToProps)(RentSearchResults);
